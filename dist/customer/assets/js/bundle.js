...
$(document).ready(function () {
  ...
  // 2025-02-06 KMVNO-5160 [DR-2025-01761] start
  accordionControl(
    '.self_failed_btn',
    '.self_failed_content',
    '.self_failed',
    ...
    '.summaryContent',
    '.summaryArea'
  ); //셀프개통 서비스 > 셀프개통 실패 사유 보기
  // 2025-02-06 KMVNO-5160 [DR-2025-01761] end
  accordionControl(
    '.accordionBtn',
    '.accordionContent',
    '.accordionArea',
...
function togglePopup(toggler, id) {
  ...
  * */
  ...
  function closePopup() {
    ...
    $('body').css('overflow', 'auto'); // 2025-04-03 [KMVNO-5374]_[DR-2025-19628]
    setTimeout(function () {
      $popup.removeClass("show");
    }, 100);
  }
...
(function() {
  const adjustPageTitlePosition = () => {
    const pageTitleContainer = document.querySelector('.pageTitleContainer');
    const headerContainer = document.querySelector('.headerContainer');
    const wrap = document.querySelector('.wrap');

    if (!pageTitleContainer || !headerContainer) return;

    const isProductPlanList = wrap?.classList.contains('productPlanList');
    const isUsimApply = wrap?.classList.contains('usim_apply');

    // usim_apply 페이지에서만 동적 처리 (NW_productPlanList는 제외)
    if (isProductPlanList && isUsimApply) {
      // topBannerContainer 높이 (있고 표시중일 때만)
      const topBannerContainer = document.querySelector('.topBannerContainer');
      const topBannerHeight = topBannerContainer && window.getComputedStyle(topBannerContainer).display !== 'none'
      ...
      const mobileNavHeight = mobileNav && window.getComputedStyle(mobileNav).display !== 'none'
        ? mobileNav.offsetHeight
        : 0;

      // topBannerContainer + .header + .mobileNav 높이를 더한 값으로 설정
      const totalHeight = topBannerHeight + headerHeight + mobileNavHeight;
      pageTitleContainer.style.top = totalHeight + 'px';
    } else if (isProductPlanList) {
      // NW_productPlanList는 기존 로직 유지 (CSS 기본값 사용)
      return;
    } else {
      // 일반 페이지는 기본 처리
      pageTitleContainer.style.top = headerContainer.offsetHeight + 'px';
    }
  };

  const initObserver = (selector, config) => {
    const observer = new MutationObserver(adjustPageTitlePosition);
    const check = setInterval(() => {
      const element = document.querySelector(selector);
      if (element) {
        observer.observe(element, config);
        clearInterval(check);
        if (selector === '.headerContainer') adjustPageTitlePosition();
      }
    }, 100);
  };

  (document.readyState === 'loading'
    ? document.addEventListener('DOMContentLoaded', () => setTimeout(adjustPageTitlePosition, 100))
    : setTimeout(adjustPageTitlePosition, 100));

  let resizeTimer;
  window.addEventListener('resize', () => {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(adjustPageTitlePosition, 100);
  });

  initObserver('.headerContainer', {
    attributes: true,
  ...
  if (typeof ResizeObserver !== 'undefined') {
    const resizeObserver = new ResizeObserver(adjustPageTitlePosition);
    ...
    // mobileMenu도 ResizeObserver로 감지
  ...
  initObserver('.wrap', { attributes: true, attributeFilter: ['class'] });
  initObserver('.topBannerContainer', { attributes: true, attributeFilter: ['style', 'class'] });
  initObserver('.mobileMenu', { attributes: true, attributeFilter: ['style', 'class'], childList: true, subtree: true });
  initObserver('.mobileThirdMenuList', { attributes: true, attributeFilter: ['style', 'class'], childList: true, subtree: true });
})();
/* header pageTitleContainer position 동적 처리 */